...
from Metrics import plot_average_auc, plot_curve
import csv
...
def run_cross_validation(data, train_data, test_data, lr, weight_decay, epochs, optimizer_type, n_splits = 5):
    ...
    edge_index = train_data['compound', 'interacts_with', 'protein'].edge_index.T
    ...
    # Loop over each fold
    for train_idx, val_idx in kf.split(edge_index):
    ...
    with open(f'test_metrics_{fileName}.csv', mode='w', newline='') as file:
